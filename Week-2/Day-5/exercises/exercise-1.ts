...
const queryUser1 = (personName: string) : Promise<User> => 
  new Promise(resolve => resolve (userData.filter((user) => user.name === personName)[ 0 ] || null));
...
const queryFood1 = (foodId:number) : Promise<Food>=> 
  new Promise (resolve => resolve (foodData.filter((food) => food.id === foodId)[ 0 ] || null));
...
const findFavouriteFood1 = (name:string) => 
  new Promise((resolve) => {
    queryUser1(name)
      .then(person1 => queryFood1(person1.food))
...
queryUser1('John').then(console.log) //{ name: 'John', food: 1 }

// Part 2: Refactor queryFood to use promises
// Test your refactored function by uncommenting the line below
queryFood1(1).then(console.log) // { name: 'Pizza', id: 1 }
...
queryUser1('John')
.then((person) => queryFood1(person.food))
